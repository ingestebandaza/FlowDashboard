-- ============================================================================
-- FlowDashboard 2.0.0 - FASE 10 - Migracion 004: RPC v2 enriquecido
-- CREATE OR REPLACE de validate_flowdashboard_license con FIRMA IDENTICA (13 params).
-- Preserva el flujo legacy (app_licenses/app_devices/app_device_registrations/app_access_logs)
-- y AÑADE: plan, limites, gracia, reason_code, espejo en license_installations,
-- gracia de expiracion (server time) y auditoria best-effort.
-- Compatibilidad: mantiene siempre status/device_status/message.
-- ============================================================================

-- Helper: mantiene license_installations en sincronia con app_devices (mismo TX, sin drift)
CREATE OR REPLACE FUNCTION public._fd_sync_installation(
  p_license_id uuid, p_device_hash text, p_hostname text, p_windows_user text,
  p_mac text, p_os text, p_app_version text, p_ip text, p_now timestamptz
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  INSERT INTO public.license_installations
    (license_id, device_hash, hostname, windows_user, mac_address, os_version, app_version, ip_public, status, first_seen_at, last_seen_at)
  VALUES
    (p_license_id, p_device_hash, nullif(p_hostname,''), nullif(p_windows_user,''), nullif(p_mac,''),
     nullif(p_os,''), nullif(p_app_version,''), nullif(p_ip,''), 'active', p_now, p_now)
  ON CONFLICT (license_id, device_hash) DO UPDATE
     SET hostname = coalesce(nullif(EXCLUDED.hostname,''), public.license_installations.hostname),
         windows_user = coalesce(nullif(EXCLUDED.windows_user,''), public.license_installations.windows_user),
         mac_address = coalesce(nullif(EXCLUDED.mac_address,''), public.license_installations.mac_address),
         os_version = coalesce(nullif(EXCLUDED.os_version,''), public.license_installations.os_version),
         app_version = coalesce(nullif(EXCLUDED.app_version,''), public.license_installations.app_version),
         ip_public = coalesce(nullif(EXCLUDED.ip_public,''), public.license_installations.ip_public),
         last_seen_at = p_now,
         status = CASE WHEN public.license_installations.status IN ('blocked','revoked')
                       THEN public.license_installations.status ELSE 'active' END;
END;
$fn$;

REVOKE ALL ON FUNCTION public._fd_sync_installation(uuid,text,text,text,text,text,text,text,timestamptz) FROM PUBLIC, anon, authenticated;
