-- ============================================================================
-- FlowDashboard 2.0.0 - FASE 11 - Migracion 006: RPCs admin transaccionales
-- Acciones criticas del panel via funciones SECURITY DEFINER que validan
-- is_app_admin() internamente y registran audit_events (actor/old-new/reason).
-- Tambien CREATE OR REPLACE del RPC de validacion para leer license_overrides
-- (gracia y limite de PCs por licencia con prioridad sobre el plan).
-- Todo reversible (rollbacks/006_rollback.sql).
-- ============================================================================

-- ---------- Helpers ----------
CREATE OR REPLACE FUNCTION public._fd_actor()
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
  SELECT coalesce(nullif(auth.jwt() ->> 'email', ''), 'panel');
$fn$;

CREATE OR REPLACE FUNCTION public._fd_require_admin()
RETURNS void
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
BEGIN
  IF NOT public.is_app_admin() THEN
    RAISE EXCEPTION 'forbidden: admin required';
  END IF;
END;
$fn$;

REVOKE ALL ON FUNCTION public._fd_actor() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public._fd_require_admin() FROM PUBLIC, anon;

DROP FUNCTION IF EXISTS public.admin_change_license_plan(uuid, bigint, text);
DROP FUNCTION IF EXISTS public.admin_publish_plan_version(bigint);
DROP FUNCTION IF EXISTS public.admin_set_plan_status(bigint, text, text);

-- ---------- Gestion de PCs ----------
CREATE OR REPLACE FUNCTION public.admin_set_device_status(
  p_device_id uuid, p_status text, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_dev public.app_devices%rowtype;
  v_now timestamptz := now();
BEGIN
  PERFORM public._fd_require_admin();
  IF p_status NOT IN ('approved','pending','blocked','revoked') THEN
    RAISE EXCEPTION 'invalid status %', p_status;
  END IF;
  SELECT * INTO v_dev FROM public.app_devices WHERE id = p_device_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'device not found'; END IF;

  UPDATE public.app_devices SET status = p_status, last_seen_at = v_now WHERE id = p_device_id;
  UPDATE public.license_installations
     SET status = CASE WHEN p_status IN ('blocked','revoked') THEN p_status ELSE 'active' END
   WHERE license_id = v_dev.license_id AND device_hash = v_dev.device_hash;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'device_status_changed', 'device', p_device_id::text,
    jsonb_build_object('old', v_dev.status, 'new', p_status, 'reason', p_reason,
      'license_id', v_dev.license_id, 'device_hash', v_dev.device_hash));

  RETURN jsonb_build_object('ok', true, 'status', p_status);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_rename_device(
  p_device_id uuid, p_name text
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_dev public.app_devices%rowtype;
BEGIN
  PERFORM public._fd_require_admin();
  IF coalesce(trim(p_name), '') = '' THEN RAISE EXCEPTION 'name required'; END IF;
  SELECT * INTO v_dev FROM public.app_devices WHERE id = p_device_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'device not found'; END IF;

  UPDATE public.app_devices SET pc_name = trim(p_name) WHERE id = p_device_id;
  UPDATE public.license_installations SET hostname = trim(p_name)
   WHERE license_id = v_dev.license_id AND device_hash = v_dev.device_hash;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'device_renamed', 'device', p_device_id::text,
    jsonb_build_object('old', v_dev.pc_name, 'new', trim(p_name)));

  RETURN jsonb_build_object('ok', true);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_unlink_device(
  p_device_id uuid, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_dev public.app_devices%rowtype;
BEGIN
  PERFORM public._fd_require_admin();
  SELECT * INTO v_dev FROM public.app_devices WHERE id = p_device_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'device not found'; END IF;

  DELETE FROM public.app_devices WHERE id = p_device_id;
  UPDATE public.license_installations SET status = 'unlinked'
   WHERE license_id = v_dev.license_id AND device_hash = v_dev.device_hash;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'device_unlinked', 'device', p_device_id::text,
    jsonb_build_object('license_id', v_dev.license_id, 'device_hash', v_dev.device_hash,
      'pc_name', v_dev.pc_name, 'reason', p_reason));

  RETURN jsonb_build_object('ok', true);
END;
$fn$;

-- ---------- Licencias ----------
CREATE OR REPLACE FUNCTION public.admin_set_license_status(
  p_license_id uuid, p_status text, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_lic public.app_licenses%rowtype;
  v_now timestamptz := now();
BEGIN
  PERFORM public._fd_require_admin();
  SELECT * INTO v_lic FROM public.app_licenses WHERE id = p_license_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'license not found'; END IF;

  UPDATE public.app_licenses SET status = p_status, updated_at = v_now WHERE id = p_license_id;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'license_status_changed', 'license', p_license_id::text,
    jsonb_build_object('old', v_lic.status, 'new', p_status, 'reason', p_reason));

  RETURN jsonb_build_object('ok', true, 'status', p_status);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_cancel_license(
  p_license_id uuid, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_lic public.app_licenses%rowtype;
  v_now timestamptz := now();
  v_devices integer := 0;
BEGIN
  PERFORM public._fd_require_admin();
  SELECT * INTO v_lic FROM public.app_licenses WHERE id = p_license_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'license not found'; END IF;

  UPDATE public.app_licenses SET status = 'cancelled', updated_at = v_now WHERE id = p_license_id;
  WITH upd AS (
    UPDATE public.app_devices SET status = 'revoked', last_seen_at = v_now
     WHERE license_id = p_license_id RETURNING 1)
  SELECT count(*) INTO v_devices FROM upd;
  UPDATE public.license_installations SET status = 'revoked' WHERE license_id = p_license_id;
  UPDATE public.license_plan_assignments SET status = 'cancelled' WHERE license_id = p_license_id;
  IF v_lic.license_key IS NOT NULL THEN
    UPDATE public.app_device_registrations SET status = 'blocked', blocked_reason = 'license_cancelled'
     WHERE license_key = v_lic.license_key;
  END IF;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'license_cancelled', 'license', p_license_id::text,
    jsonb_build_object('old', v_lic.status, 'new', 'cancelled', 'reason', p_reason, 'devices_revoked', v_devices));

  RETURN jsonb_build_object('ok', true, 'devices_revoked', v_devices);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_change_license_plan(
  p_license_id uuid, p_plan_version_id uuid, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_plan_id uuid;
  v_old jsonb;
BEGIN
  PERFORM public._fd_require_admin();
  SELECT plan_id INTO v_plan_id FROM public.plan_versions WHERE id = p_plan_version_id;
  IF v_plan_id IS NULL THEN RAISE EXCEPTION 'plan version not found'; END IF;

  SELECT jsonb_build_object('plan_id', plan_id, 'plan_version_id', plan_version_id, 'status', status)
    INTO v_old FROM public.license_plan_assignments WHERE license_id = p_license_id;

  INSERT INTO public.license_plan_assignments (license_id, plan_id, plan_version_id, status)
  VALUES (p_license_id, v_plan_id, p_plan_version_id, 'active')
  ON CONFLICT (license_id) DO UPDATE
    SET plan_id = EXCLUDED.plan_id, plan_version_id = EXCLUDED.plan_version_id, status = 'active';

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'license_plan_changed', 'license', p_license_id::text,
    jsonb_build_object('old', v_old, 'new',
      jsonb_build_object('plan_id', v_plan_id, 'plan_version_id', p_plan_version_id), 'reason', p_reason));

  RETURN jsonb_build_object('ok', true, 'plan_id', v_plan_id, 'plan_version_id', p_plan_version_id);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_set_license_override(
  p_license_id uuid, p_key text, p_value jsonb, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_old jsonb;
BEGIN
  PERFORM public._fd_require_admin();
  IF p_key NOT IN ('max_pc_installations','offline_grace_hours','expiration_grace_hours') THEN
    RAISE EXCEPTION 'invalid override key %', p_key;
  END IF;

  SELECT value INTO v_old FROM public.license_overrides WHERE license_id = p_license_id AND key = p_key;

  IF p_value IS NULL OR jsonb_typeof(p_value) = 'null' THEN
    DELETE FROM public.license_overrides WHERE license_id = p_license_id AND key = p_key;
  ELSE
    INSERT INTO public.license_overrides (license_id, key, value)
    VALUES (p_license_id, p_key, p_value)
    ON CONFLICT (license_id, key) DO UPDATE SET value = EXCLUDED.value;
  END IF;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'license_override_set', 'license', p_license_id::text,
    jsonb_build_object('key', p_key, 'old', v_old, 'new', p_value, 'reason', p_reason));

  RETURN jsonb_build_object('ok', true);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_record_payment(
  p_license_id uuid, p_amount numeric, p_currency text DEFAULT 'USD',
  p_method text DEFAULT NULL, p_reference text DEFAULT NULL,
  p_period_start date DEFAULT NULL, p_period_end date DEFAULT NULL,
  p_paid_at timestamptz DEFAULT now(), p_notes text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_id uuid;
BEGIN
  PERFORM public._fd_require_admin();
  IF NOT EXISTS (SELECT 1 FROM public.app_licenses WHERE id = p_license_id) THEN
    RAISE EXCEPTION 'license not found';
  END IF;

  INSERT INTO public.payments
    (license_id, amount, currency, method, reference, period_start, period_end, paid_at, notes, created_by)
  VALUES
    (p_license_id, p_amount, coalesce(p_currency,'USD'), p_method, p_reference,
     p_period_start, p_period_end, coalesce(p_paid_at, now()), p_notes, public._fd_actor())
  RETURNING id INTO v_id;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'payment_recorded', 'license', p_license_id::text,
    jsonb_build_object('payment_id', v_id, 'amount', p_amount, 'currency', coalesce(p_currency,'USD')));

  RETURN jsonb_build_object('ok', true, 'payment_id', v_id);
END;
$fn$;

-- ---------- Plan Studio: publicar / archivar version ----------
CREATE OR REPLACE FUNCTION public.admin_publish_plan_version(
  p_plan_version_id uuid
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_plan_id uuid;
BEGIN
  PERFORM public._fd_require_admin();
  SELECT plan_id INTO v_plan_id FROM public.plan_versions WHERE id = p_plan_version_id;
  IF v_plan_id IS NULL THEN RAISE EXCEPTION 'plan version not found'; END IF;

  UPDATE public.plan_versions SET is_current = (id = p_plan_version_id) WHERE plan_id = v_plan_id;
  UPDATE public.plans SET status = 'active' WHERE id = v_plan_id AND status <> 'active';

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'plan_version_published', 'plan', v_plan_id::text,
    jsonb_build_object('plan_version_id', p_plan_version_id));

  RETURN jsonb_build_object('ok', true, 'plan_id', v_plan_id);
END;
$fn$;

CREATE OR REPLACE FUNCTION public.admin_set_plan_status(
  p_plan_id uuid, p_status text, p_reason text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $fn$
DECLARE
  v_old text;
BEGIN
  PERFORM public._fd_require_admin();
  IF p_status NOT IN ('active','draft','retired') THEN
    RAISE EXCEPTION 'invalid plan status %', p_status;
  END IF;
  SELECT status INTO v_old FROM public.plans WHERE id = p_plan_id;
  IF v_old IS NULL THEN RAISE EXCEPTION 'plan not found'; END IF;

  UPDATE public.plans SET status = p_status WHERE id = p_plan_id;

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES (public._fd_actor(), 'plan_status_changed', 'plan', p_plan_id::text,
    jsonb_build_object('old', v_old, 'new', p_status, 'reason', p_reason));

  RETURN jsonb_build_object('ok', true, 'status', p_status);
END;
$fn$;

-- ---------- Grants de los RPCs admin (RLS-independiente; protegidos por _fd_require_admin) ----------
DO $$
DECLARE
  fn text;
  fns text[] := ARRAY[
    'admin_set_device_status(uuid,text,text)',
    'admin_rename_device(uuid,text)',
    'admin_unlink_device(uuid,text)',
    'admin_set_license_status(uuid,text,text)',
    'admin_cancel_license(uuid,text)',
    'admin_change_license_plan(uuid,uuid,text)',
    'admin_set_license_override(uuid,text,jsonb,text)',
    'admin_record_payment(uuid,numeric,text,text,text,date,date,timestamptz,text)',
    'admin_publish_plan_version(uuid)',
    'admin_set_plan_status(uuid,text,text)'];
BEGIN
  FOREACH fn IN ARRAY fns LOOP
    EXECUTE format('REVOKE ALL ON FUNCTION public.%s FROM PUBLIC, anon;', fn);
    EXECUTE format('GRANT EXECUTE ON FUNCTION public.%s TO authenticated;', fn);
  END LOOP;
END $$;

-- ---------- RPC de validacion: leer license_overrides (prioridad sobre el plan) ----------
CREATE OR REPLACE FUNCTION public.validate_flowdashboard_license(
  p_device_email   text,
  p_license_key    text,
  p_device_hostname text DEFAULT ''::text,
  p_device_serial  text DEFAULT ''::text,
  p_device_os      text DEFAULT ''::text,
  p_ip_public      text DEFAULT ''::text,
  p_country_code   text DEFAULT ''::text,
  p_device_hash    text DEFAULT ''::text,
  p_windows_user   text DEFAULT ''::text,
  p_app_version    text DEFAULT ''::text,
  p_local_ip       text DEFAULT ''::text,
  p_mac_address    text DEFAULT ''::text,
  p_country_name   text DEFAULT ''::text
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_license       public.app_licenses%rowtype;
  v_device        public.app_devices%rowtype;
  v_registration  public.app_device_registrations%rowtype;
  v_now           timestamptz := now();
  v_message       text := '';
  v_device_count  integer := 0;
  v_max_devices   integer := 1;
  v_plan_code     text := NULL;
  v_plan_name     text := NULL;
  v_max_pc        integer := NULL;
  v_max_android   integer := NULL;
  v_max_concurrent integer := NULL;
  v_offline_grace integer := 72;
  v_exp_grace     integer := 48;
  v_ovr_pc        integer := NULL;
  v_ovr_offline   integer := NULL;
  v_ovr_exp       integer := NULL;
  v_extra         jsonb;
  v_grace_flag    boolean := false;
BEGIN
  p_device_email := lower(trim(coalesce(p_device_email, '')));
  p_license_key := trim(coalesce(p_license_key, ''));
  p_device_hostname := trim(coalesce(p_device_hostname, ''));
  p_windows_user := trim(coalesce(p_windows_user, ''));
  p_device_hash := trim(coalesce(p_device_hash, ''));
  p_app_version := trim(coalesce(p_app_version, ''));
  p_local_ip := trim(coalesce(p_local_ip, ''));
  p_mac_address := trim(coalesce(p_mac_address, ''));
  p_country_name := trim(coalesce(p_country_name, ''));

  IF p_device_hash = '' THEN
    p_device_hash := md5(p_device_email || '|' || p_device_hostname || '|' || p_windows_user);
  END IF;

  IF p_device_email = '' OR p_license_key = '' THEN
    RETURN jsonb_build_object('status', 'error', 'device_status', 'missing',
      'reason_code', 'missing_input', 'schema_version', 2, 'message', 'Email y licencia requeridos');
  END IF;

  SELECT * INTO v_license FROM public.app_licenses WHERE license_key = p_license_key LIMIT 1;

  IF NOT FOUND THEN
    v_message := 'Licencia invalida';
    INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
    VALUES (v_now, v_now, p_license_key, p_device_email, 'license_not_found', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);
    RETURN jsonb_build_object('status', 'error', 'device_status', 'missing',
      'reason_code', 'license_not_found', 'schema_version', 2, 'message', v_message);
  END IF;

  SELECT p.code, p.name, v.max_pc_installations, v.max_android_devices,
         v.max_concurrent_android_devices, v.offline_grace_hours, v.expiration_grace_hours
    INTO v_plan_code, v_plan_name, v_max_pc, v_max_android, v_max_concurrent, v_offline_grace, v_exp_grace
    FROM public.license_plan_assignments a
    JOIN public.plans p ON p.id = a.plan_id
    JOIN public.plan_versions v ON v.id = a.plan_version_id
   WHERE a.license_id = v_license.id
   LIMIT 1;

  v_offline_grace := coalesce(v_offline_grace, 72);
  v_exp_grace := coalesce(v_exp_grace, 48);

  SELECT
    max(CASE WHEN key = 'max_pc_installations' THEN (value #>> '{}')::int END),
    max(CASE WHEN key = 'offline_grace_hours' THEN (value #>> '{}')::int END),
    max(CASE WHEN key = 'expiration_grace_hours' THEN (value #>> '{}')::int END)
    INTO v_ovr_pc, v_ovr_offline, v_ovr_exp
    FROM public.license_overrides WHERE license_id = v_license.id;

  IF v_ovr_pc IS NOT NULL THEN v_max_pc := v_ovr_pc; END IF;
  IF v_ovr_offline IS NOT NULL THEN v_offline_grace := v_ovr_offline; END IF;
  IF v_ovr_exp IS NOT NULL THEN v_exp_grace := v_ovr_exp; END IF;

  v_extra := jsonb_build_object(
    'schema_version', 2,
    'plan', jsonb_build_object('code', v_plan_code, 'name', v_plan_name),
    'limits', jsonb_build_object(
      'max_pc_installations', v_max_pc,
      'max_android_devices', v_max_android,
      'max_concurrent_android_devices', v_max_concurrent),
    'grace', jsonb_build_object(
      'offline_hours', v_offline_grace,
      'expiration_hours', v_exp_grace)
  );

  IF lower(coalesce(v_license.status, '')) NOT IN ('active', 'approved') THEN
    v_message := 'Licencia ' || coalesce(v_license.status, 'inactiva');
    INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
    VALUES (v_now, v_now, p_license_key, p_device_email, 'license_inactive', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);
    RETURN jsonb_build_object('status', 'error', 'device_status', lower(coalesce(v_license.status, 'inactive')),
      'reason_code', 'license_inactive', 'message', v_message) || v_extra;
  END IF;

  IF v_license.expires_at IS NOT NULL THEN
    IF v_license.expires_at + make_interval(hours => v_exp_grace) < v_now THEN
      v_message := 'Licencia expirada';
      INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
      VALUES (v_now, v_now, p_license_key, p_device_email, 'license_expired', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);
      RETURN jsonb_build_object('status', 'error', 'device_status', 'expired',
        'reason_code', 'license_expired', 'message', v_message) || v_extra;
    ELSIF v_license.expires_at < v_now THEN
      v_grace_flag := true;
    END IF;
  END IF;

  v_max_devices := greatest(coalesce(v_max_pc, v_license.max_devices, 1), 1);

  SELECT * INTO v_device FROM public.app_devices
   WHERE license_id = v_license.id AND device_hash = p_device_hash LIMIT 1;

  IF FOUND THEN
    IF lower(coalesce(v_device.status, '')) IN ('blocked', 'revoked') THEN
      v_message := 'PC bloqueada';
      INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
      VALUES (v_now, v_now, p_license_key, p_device_email, 'device_blocked', v_message, 'blocked', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);
      RETURN jsonb_build_object('status', 'error', 'device_status', 'blocked',
        'reason_code', 'device_blocked', 'message', v_message) || v_extra;
    END IF;

    UPDATE public.app_devices
       SET pc_name = coalesce(nullif(p_device_hostname, ''), pc_name),
           windows_user = coalesce(nullif(p_windows_user, ''), windows_user),
           ip = coalesce(nullif(p_ip_public, ''), ip),
           local_ip = coalesce(nullif(p_local_ip, ''), local_ip),
           mac_address = coalesce(nullif(p_mac_address, ''), mac_address),
           country = coalesce(nullif(p_country_code, ''), country),
           country_name = coalesce(nullif(p_country_name, ''), country_name),
           app_version = coalesce(nullif(p_app_version, ''), app_version),
           last_seen_at = v_now
     WHERE id = v_device.id;

    PERFORM public._fd_sync_installation(v_license.id, p_device_hash, p_device_hostname,
      p_windows_user, p_mac_address, p_device_os, p_app_version, p_ip_public, v_now);

    v_message := CASE WHEN v_grace_flag THEN 'Dispositivo aprobado (gracia de expiracion)' ELSE 'Dispositivo aprobado' END;
    INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
    VALUES (v_now, v_now, p_license_key, p_device_email, 'device_seen', NULL, 'approved', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);

    RETURN jsonb_build_object('status', 'success', 'device_status', coalesce(v_device.status, 'approved'),
      'reason_code', CASE WHEN v_grace_flag THEN 'expiration_grace' ELSE 'device_seen' END, 'message', v_message) || v_extra;
  END IF;

  SELECT count(*) INTO v_device_count FROM public.app_devices
   WHERE license_id = v_license.id
     AND lower(coalesce(status, 'approved')) NOT IN ('blocked', 'revoked');

  IF v_device_count >= v_max_devices THEN
    v_message := 'Limite de PCs alcanzado';
    INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
    VALUES (v_now, v_now, p_license_key, p_device_email, 'device_limit_reached', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);
    RETURN jsonb_build_object('status', 'error', 'device_status', 'limit_reached',
      'reason_code', 'pc_limit_reached', 'message', v_message) || v_extra;
  END IF;

  INSERT INTO public.app_devices (license_id, pc_name, windows_user, device_hash, ip, local_ip, mac_address, country, country_name, app_version, status, created_at, last_seen_at)
  VALUES (v_license.id, p_device_hostname, p_windows_user, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, 'approved', v_now, v_now);

  PERFORM public._fd_sync_installation(v_license.id, p_device_hash, p_device_hostname,
    p_windows_user, p_mac_address, p_device_os, p_app_version, p_ip_public, v_now);

  SELECT * INTO v_registration FROM public.app_device_registrations
   WHERE device_email = p_device_email AND license_key = p_license_key LIMIT 1;

  IF FOUND THEN
    UPDATE public.app_device_registrations
       SET last_seen_at = v_now,
           device_hostname = coalesce(nullif(p_device_hostname, ''), device_hostname),
           device_serial = coalesce(nullif(p_device_serial, ''), device_serial),
           device_os = coalesce(nullif(p_device_os, ''), device_os),
           ip_public = coalesce(nullif(p_ip_public, ''), ip_public),
           local_ip = coalesce(nullif(p_local_ip, ''), local_ip),
           mac_address = coalesce(nullif(p_mac_address, ''), mac_address),
           country_code = coalesce(nullif(p_country_code, ''), country_code),
           country_name = coalesce(nullif(p_country_name, ''), country_name),
           status = 'approved'
     WHERE id = v_registration.id;
  ELSE
    INSERT INTO public.app_device_registrations (license_key, device_email, device_hostname, device_serial, device_os, ip_public, local_ip, mac_address, country_code, country_name, status, created_at, last_seen_at)
    VALUES (p_license_key, p_device_email, p_device_hostname, p_device_serial, p_device_os, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, 'approved', v_now, v_now);
  END IF;

  v_message := CASE WHEN v_grace_flag THEN 'Dispositivo aprobado (gracia de expiracion)' ELSE 'Dispositivo aprobado' END;
  INSERT INTO public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, local_ip, mac_address, country, country_name, app_version, message)
  VALUES (v_now, v_now, p_license_key, p_device_email, 'device_registered', NULL, 'approved', p_device_hostname, p_device_hash, p_ip_public, p_local_ip, p_mac_address, p_country_code, p_country_name, p_app_version, v_message);

  INSERT INTO public.audit_events (actor, action, entity_type, entity_id, details)
  VALUES ('rpc', 'device_registered', 'license', v_license.id::text,
    jsonb_build_object('device_hash', p_device_hash, 'email', p_device_email, 'plan', v_plan_code));

  RETURN jsonb_build_object('status', 'success', 'device_status', 'approved',
    'reason_code', CASE WHEN v_grace_flag THEN 'expiration_grace' ELSE 'device_registered' END, 'message', v_message) || v_extra;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.validate_flowdashboard_license(text,text,text,text,text,text,text,text,text,text,text,text,text) TO anon, authenticated;

INSERT INTO flow_backup_v2.migration_meta (step, notes)
VALUES ('006_admin_rpcs', 'Admin transactional RPCs + audit; validate RPC reads license_overrides');
