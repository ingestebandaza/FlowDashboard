...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$manifest=[ordered]@{version='1.0.60'; package_url='https://github.com/ingestebandaza/FlowDashboard/releases/download/v1.0.60/FlowDashboard-1.0.60.zip'; sha256='%SHA256%'}; $json=ConvertTo-Json -InputObject $manifest; [System.IO.File]::WriteAllText('%APP_DIR%\update.json', $json + [Environment]::NewLine, [System.Text.UTF8Encoding]::new($false))"
...
echo.
echo Siguiente paso:
echo 1. Sube este ZIP a una Release de GitHub con tag v1.0.60.
