...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$release='%RELEASE_DIR%'; $staging='%STAGING_DIR%'; $null=New-Item -ItemType Directory -Force -Path $release; if (Test-Path -LiteralPath $staging) { Remove-Item -LiteralPath $staging -Recurse -Force }; $null=New-Item -ItemType Directory -Force -Path $staging; Copy-Item -LiteralPath '%DIST_DIR%\FlowDashboard.exe' -Destination $staging -Force; foreach ($file in @('wsapi_demo.html','wsapi.js','Login.js','logo.png')) { $src=Join-Path '%APP_DIR%' $file; if (Test-Path -LiteralPath $src) { Copy-Item -LiteralPath $src -Destination $staging -Force } }; if (Test-Path -LiteralPath '%APP_DIR%\.supabase_config.json') { Copy-Item -LiteralPath '%APP_DIR%\.supabase_config.json' -Destination $staging -Force }; if (Test-Path -LiteralPath '%APP_DIR%\update_config.json') { Copy-Item -LiteralPath '%APP_DIR%\update_config.json' -Destination $staging -Force }; if (Test-Path -LiteralPath '%ZIP_PATH%') { Remove-Item -LiteralPath '%ZIP_PATH%' -Force }; Compress-Archive -Path ($staging + '\*') -DestinationPath '%ZIP_PATH%' -Force"
if errorlevel 1 (
