...
begin
  ...
  if not found then
    ...
    insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
    values (v_now, v_now, p_license_key, p_device_email, 'license_not_found', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
  ...
  if lower(coalesce(v_license.status, '')) not in ('active', 'approved') then
    ...
    insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
    values (v_now, v_now, p_license_key, p_device_email, 'license_inactive', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
  ...
  if v_license.expires_at is not null and v_license.expires_at < v_now then
    ...
    insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
    values (v_now, v_now, p_license_key, p_device_email, 'license_expired', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
  ...
  if found then
    if lower(coalesce(v_device.status, '')) in ('blocked', 'revoked') then
      ...
      insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
      values (v_now, v_now, p_license_key, p_device_email, 'device_blocked', v_message, 'blocked', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
    ...
    insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
    values (v_now, v_now, p_license_key, p_device_email, 'device_seen', null, 'approved', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
  ...
  if v_device_count >= v_max_devices then
    ...
    insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
    values (v_now, v_now, p_license_key, p_device_email, 'device_limit_reached', v_message, 'denied', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);
  ...
  v_message := 'Dispositivo aprobado';
  insert into public.app_access_logs (created_at, "timestamp", license_key, device_email, event_type, error_message, decision, pc_name, device_hash, ip, country, app_version, message)
  values (v_now, v_now, p_license_key, p_device_email, 'device_registered', null, 'approved', p_device_hostname, p_device_hash, p_ip_public, p_country_code, p_app_version, v_message);

  return jsonb_build_object('status', 'success', 'device_status', 'approved', 'message', v_message);
end;
$$;

revoke all on function public.validate_flowdashboard_license(text, text, text, text, text, text, text, text, text, text) from public;
grant execute on function public.validate_flowdashboard_license(text, text, text, text, text, text, text, text, text, text) to anon;
grant execute on function public.validate_flowdashboard_license(text, text, text, text, text, text, text, text, text, text) to authenticated;
